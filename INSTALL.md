...
* The **OCaml** LTS (Long Term Support) release **4.14.x**
  ...
  * ***Case 3: OCaml is installed with the compiler's LTS release***
    ...
    ```
> [!TIP]
> _If you want to avoid typing_ `opam switch set LTS` _in every bash session, you can add it to your_ `~/.bashrc` _script._
---
...
| `-y`, `--yes` | *None* | Assume 'yes' as answer to the "install ACME?" prompt |
| `-h`, `--help` | *None* | Show help and default installation paths |
> [!NOTE]
> _Entering no options will cause the O64ml system to be installed in the current Opam switch **(recommended)**._
...
> [!IMPORTANT]
> _If you ran_ `./configure` _with customized directory options, make sure you have appropriate permissions on target directories. Also ensure that the O64ml final users have appropriate permissions on all the installed files._

> [!NOTE]
> _If you have an obsolete ACME version in a directory other than the_ `-b <dir>` _(or the default binary installation directory),_ `./configure` _will put the newer ACME release in the latter, leaving the older version in its place; O64ml will use the appropriate one._
#### 4. Clean up
...
```
> [!WARNING]
> _If the configuration file_ `./etc/Makefile.conf` _is missing, the uninstallation will fail. If so, re-run_ `./configure` _with the same options you entered at installation time, then try to uninstall O64ml again._
