...
;; ----------------------------------------------------------------------------
;; Some peculiarities of the bytecode emitted by ocamlc:
;;  * opcode RETURN:
;; 	A function might be called with more arguments on the stack than
;; 	expected. If there is any extra argument, RETURN does not give control
;; 	back to the caller: instead, it assumes that there is a closure in the
;; 	accumulator and gives it immediate control.
;;  * opcodes RESTART and GRAB:
;; 	GRAB checks that the caller passed enough arguments onto the stack. If
;; 	not, it creates a closure, stores all the available arguments in it,
;; 	points the closure code at the instruction just before GRAB (which is
;; 	assumed	to be a RESTART), then returns to the caller, as if executing a
;; 	RETURN instruction. When the new closure is called, RESTART will push
;; 	the arguments onto the stack again and fallthrough GRAB.
;;  * opcodes PUSHRETADDR and APPLY:
...
;; 	followed by instructions that compute and push the arguments, and
;; 	finally an APPLY n instruction that jump to the closure code. When n<4,
;; 	ocamlc skips the PUSHRETADDR: pushing the return address onto the stack
;; 	is left up to APPLY1 (or APPLY2, APPLY3).
;; ----------------------------------------------------------------------------
...
+	DEC SP					;Decrement the stack pointer, lo
  ...
  !ifdef caml_INTERP {
		RTS				;No 'JMP caml_interp_fetch' here!
  } else {
	RTS				;{X = x, Y = 0}
  }
}
...
-	LDA (SP),Y				;Field(ACCU, i+2) := SP[i],
	...
	BCC +
	INC SP + 1
+	;; fallthrough RETFRAME		;{Y = 0}
}

RETFRAME					;NATIVE CODE & INTERPRETER
!ifdef caml_gen_RETFRAME {
  !ifdef caml_INTERP {
		@PC = PC
  } else {				;{}
	@PC = TMP
...
+	LDA XARGS				;If XARGS - Val_one < Val_zero
	...
	BCC RETFRAME				;then get return frame
...
!ifdef caml_gen_GETGLB {
  !ifdef caml_INTERP {
		JSR GETGLBSUB
		JMP caml_interp_fetch
GETGLBSUB					;called by [PH]GETGLBFLD too
		JSR caml_interp_getarg
		STA ACCU
		JSR caml_interp_getarg
		STA ACCU + 1
...
!ifdef caml_gen_MKBLK13 {
  !ifdef caml_INTERP {
		SBC # $7A -1			;-1 as C=0; A:=2n=2*opc-$7A
		LSR
		TAX				;X = size
		JSR caml_interp_getarg		;A = tag
  }		;; fallthrough MKBLK
...
@jmp	JMP (.dummy)				;SMC: JMP (adr + 2n + 2i)
  }
}					;{Y = 0}
...
ADDINT		       				;NATIVE CODE & INTERPRETER
	;; Val_Int(m + n) = Val_Int(m) - 1 + Val_Int(n)
!ifdef caml_gen_ADDINT {		;{Y = 0}
	LDA (SP),Y				;Load SP[0], lo byte
	AND # $FE				;Decrement it by clearing LSB
	CLC					;Prepare ADC
	ADC ACCU				;Add ACCU, lo byte, to m
	STA ACCU				;and save result
	INC SP					;Pop read byte from stack
	LDA (SP),Y				;Load SP[0], hi byte
	ADC ACCU + 1				;Add ACCU+1 to it
	STA ACCU + 1				;and save result
	INC SP					;Pop read byte from stack
	BNE +					;If page crossing
	INC SP + 1				;increment stack ptr, hi byte
+
  !ifdef caml_INTERP {
		JMP caml_interp_fetch
  } else {
...
SUBINT						;NATIVE CODE & INTERPRETER
	;; Val_Int(m - n) = Val_Int(m) - Val_Int(n) + 1
!ifdef caml_gen_SUBINT {		;{Y = 0}
	LDA ACCU				;Load ACCU
	SEC					;Prepare SBC
	SBC (SP),Y				;Subtract SP[0], lo byte
	ORA # 1					;Add 1 by setting LSB
	STA ACCU				;and save result
	INC SP					;Pop read byte from stack
	LDA ACCU + 1				;Load ACCU+1
	SBC (SP),Y				;Subtract SP[0], hi byte
	STA ACCU + 1				;and save result
	INC SP					;Pop read byte from stack
	BNE +					;If page crossing
	INC SP + 1				;increment stack ptr, hi byte
...
ANDINT						;NATIVE CODE & INTERPRETER
	;; Val_Int(m & n) = Val_Int(m) & Val_Int(n)
!ifdef caml_gen_ANDINT {		;{Y = 0}
	LDA (SP),Y
	AND ACCU
	STA ACCU
	INC SP
...
ORINT						;NATIVE CODE & INTERPRETER
	;; Val_Int(m | n) = Val_Int(m) | Val_Int(n)
!ifdef caml_gen_ORINT {			;{Y = 0}
	LDA (SP),Y
	ORA ACCU
	STA ACCU
	INC SP
...
XORINT						;NATIVE CODE & INTERPRETER
	;; Val_Int(m xor n) = Val_Int(m) xor Val_Int(n) + 1
!ifdef caml_gen_XORINT {		;{Y = 0}
	LDA (SP),Y
	EOR ACCU
	ORA # 1
	STA ACCU
...
+	LSR					;compute b from Val_Int(b)
	...
	TAX					;X := b
	LDA ACCU				;load ACCU, lo
	AND # $FE				;clear lsb
-	ASL					;loop:	shift left ACCU, lo
	ROL ACCU + 1				;	shift left ACCU, hi
	DEX
	BNE -					;repeat b times
	ORA # 1					;set lsb
	STA ACCU				;save ACCU, lo
+
  !ifdef caml_INTERP {
		JMP caml_interp_fetch
  } else {
...
!ifdef caml_gen_ASRINT {		;{Y = 0}
	BIT ACCU + 1				;test sign
	BPL LSRINT				;+: Jump to lsr routine
	LDA (SP),Y				;load Val_Int(b)
	INC SP					;and pop it
	INC SP
	BNE +
...
+	LSR					;compute b from Val_Int(b)
	...
	TAX					;X := b
	LDA ACCU				;load ACCU, lo
-	SEC					;loop:	preserve sign bit
	ROR ACCU + 1				; shift right ACCU, hi
	ROR					; shift right ACCU, lo
	DEX
	BNE -					;repeat b times
	ORA # 1					;set lsb
	STA ACCU				;save ACCU, lo
...
!ifdef caml_gen_LSRINT {		;{Y = 0}
	...
	INC SP + 1
+	LSR					;compute b from Val_Int(b)
	AND # $0F				;extract lo nibble from b
	BEQ +					;=0? nothing to be shifted
	TAX					;X := b
	LDA ACCU				;load ACCU, lo
-	LSR ACCU + 1				;loop:	shift right ACCU, hi
	ROR					;	shift right ACCU, lo
	DEX
	BNE -					;repeat b times
	ORA # 1					;set lsb
...
CMPTAIL						;NATIVE CODE & INTERPRETER
	;; EQ, NEQ, [LG][ET]INT - shared code
!ifdef caml_gen_CMPTAIL {		;{Y = 0}
CMPTAIL_T
	LDA # Val_true
	BNE +					;JMP ++
CMPTAIL_F
