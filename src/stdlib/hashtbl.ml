...
(*--
let randomized_default =  let params =
...
let randomize () = failwith "randomize"
...
--*)
(*++ *)
let create ?(random = false) initial_size =
  let s = power_2_above 16 initial_size in
...
(*++ *)
let reset h =
  if Array.length h.data = abs h.initial_size then
    clear h
  else begin
    h.size <- 0;
    h.data <- Array.make (abs h.initial_size) Empty
...
--*)
(*++ *)
    let find_opt h key = find_rec_opt key h.data.(key_index h key)
(* ++*)
...
(* ++*)
  struct
    ...
    let create ?(random=false) sz = create sz
    let of_seq i =
      let tbl = create 16 in
      replace_seq tbl i;
      tbl
...
--*)
(*++ *)
external hash :'a -> int = "caml_hash"
let seeded_hash _seed x = hash x
let hash_param _n1 _n2 x = hash x
let seeded_hash_param _m _tot _seed x = hash x
(* ++*)
...
--*)
(*++ *)
let key_index h key = (hash key) land (Array.length h.data - 1)
(* ++*)
