...
(*++ *)
(* The PRNG is implemented in assembly. *)
external random_self_init : unit -> unit = "caml_nonstd_random_self_init"
external random_int : unit -> int = "caml_nonstd_random_int"
external random_set_state : bytes -> unit = "caml_nonstd_random_set_state"
external random_get_state : unit -> bytes = "caml_nonstd_random_get_state"

let bytes_of_seed seed =
  let xabc = Bytes.make 4 '\000' in
  let char n = char_of_int (n land 0xFF) in
  for i = 0 to Array.length seed - 1 do
    let j = i mod 4 in
    let new_char = char (seed.(i) lxor (Bytes.get_uint8 xabc j)) in
    Bytes.set xabc j new_char
  done;
  xabc

(* Basic random functions.
   They are faster than the advanced ones, but the latter permit to use
   many independent PRNGs, each of them initialised with its own seed.
   TO SPEED UP ADV.FUNCS AT COST OF BASIC ONES, READ THE COMMENTS BELOW *)

let full_init seed = random_set_state (bytes_of_seed seed)
let init n = random_set_state (bytes_of_seed [| n land 0xFF; n lsr 8|])
let self_init () = random_self_init ()
let bits () = abs (random_int ())
let int bound =
  if bound <= 0 then invalid_arg "Random.int"
  else bits () mod bound
let full_int = int
(* removed Int32 and Int64 dependances
let bits32 () =
  let shift b = Int32.shift_left b 15 in
  let bits15 () = Int32.of_int (random_int ()) in
  let bits2 () = Int32.of_int (random_int () land 0b11) in
  bits15 ()
  |> shift |> Int32.logor (bits15 ())
  |> shift |> Int32.logor (bits2 ())
let bits64 () =
  let shift b = Int64.shift_left b 15 in
  let bits15 () = Int64.of_int (random_int ()) in
  let bits4 () = Int64.of_int (random_int () land 0b1111) in
  bits15 ()
  |> shift |> Int64.logor (bits15 ())
  |> shift |> Int64.logor (bits15 ())
  |> shift |> Int64.logor (bits15 ())
  |> shift |> Int64.logor (bits4 ())
let int32 bound =
  if bound <= 0l then invalid_arg "Random.int32"
  else Int32.(rem (abs (bits32 ())) bound)
let int64 bound =
    if bound <= 0L then invalid_arg "Random.int32"
    else Int64.(rem (abs (bits64 ())) bound)
 *)
let float (_n : float) = failwith "Random.float non implemented yet"
let bool () = random_int () land 1 = 1

(* Advanced functions - independent PRNGs *)
module State = struct
  type t = bytes ref
  let make seed = ref (bytes_of_seed seed)
  let make_self_init () =
    random_self_init ();
    ref (random_get_state ())
  let copy state = ref !state
  let call_with_state f state a =

    (* TO SPEED UP ADV.FUNCS AT COST OF BASIC ONES, COMMENT OUT THIS LINE *)
    let curr_state = random_get_state () in
    ...
    (* TO SPEED UP ADV.FUNCS AT COST OF BASIC ONES, COMMENT OUT THIS LINE *)
    random_set_state curr_state;

    result
  let bits state = call_with_state bits state ()
  let bool state = call_with_state bool state ()
  let int = call_with_state int 
  let full_int = call_with_state full_int
(* removed Int32 and Int64 dependances
  let bits32 state = call_with_state bits32 state ()
  let bits64 state = call_with_state bits64 state ()
  let int32 = call_with_state int32 
  let int64 = call_with_state int64 
*)
  let float = call_with_state float 
end

(* TO SPEED UP ADV.FUNCS AT COST OF BASIC ONES, UNCOMMENT THE FOLLOWING LINES
   let default = State.make [|0;0;0;0|]
   let full_init seed = default := !(State.make seed)
   let init n = default := !(State.make [| n land 0xFF; n lsr 8|])
   let self_init () = default := !(State.make_self_init ())
   let bits = State.bits default
   let bool = State.bool default
   let int = State.int default
   let full_int = State.full_int default
   let bits32 = State.bits32 default
   let bits64 = State.bits64 default
   let int32 = State.int32 default
   let int64 = State.int64 default
   let float = State.float default
 *)

let get_state () = ref (random_get_state ())
let set_state state = random_set_state !state
(* ++*)
