...
let input_files = ref []	(* input files *)
...
let db = ref "bcamlppx.db"	(* -db option *)
  ...
  let cmd =
    ...
    String.concat " "
      (("CAMLLIB=" ^ quote Config.libdir)
       ...
       :: "-ppx"
       :: quote (Config.bindir ^ "/bcamlppx " ^ !db)
       :: !ocamlc_opts
...
let usage = Printf.sprintf
 ("Usage:\n\
   ...
   \n\
   Compile and links the given FILEs into an executable file for Commodore 64\n\
   computers.\n\
...
let bcamlc_opts =
  ...
  Arg.["-o", Set_string o_arg,
       ...
       ^ nl_indent "'asm', generate the assembly file 'outfile.asm'; if .ext is"
       ^ nl_indent "'prg', also assemble it into the executable 'outfile.prg'.";
       "-c", Set compile_only,
       " Compile only.";
       "-interp", Set interp,
       ...
       ^ nl_indent (Printf.sprintf "Default: %#4x (%5d)." !mem_arg !mem_arg);
       "-stack", Set_int stack_arg,
       "pages Define the stack size, in 256-byte pages."
       ^ nl_indent (Printf.sprintf "Default: %d." !stack_arg);
       "-showmem", Set showmem, " Show information on memory allocation.";
       "-db", Set_string db,
       "filename Set the filename for the preprocessor database.";
...
let rec dyn_add_ocamlc_opts acc n =
  (* dynamically add any non-bcamlc options to the ocamlc ones *)
